...
**Время выполнения:** 2–3 часа. **Стек:** Python, Django, DRF, Celery (Redis), PostgreSQL.
...
* **Race Condition Check:** Представь, что на этот эндпоинт приходит 100 запросов в секунду от одного юзера с целью списать баланс в минус. Реализуй защиту от **Double Spending** (двойного списания). Подсказка: Просто `if balance > amount` недостаточно.
* **Комиссия системы:** Если сумма перевода $> 1000$ u., система берет комиссию 10%. Комиссия должна зачисляться на специальный технический кошелек `admin`. Все три операции (списание, зачисление, комиссия) должны пройти **атомарно** (либо все, либо ничего).
...
* Создай задачу в **Celery**.
* Внутри задачи сделай имитацию долгого запроса (`time.sleep(5)`).
* **Важно:** Если "отправка" упала (симуляция ошибки), задача должна автоматически перезапуститься (**Retry**) через 3 секунды, но не более 3-х раз.

---

## 📹 Задача 3: Видео-защита (Screencast)

Запиши короткое видео (3-5 минут) с демонстрацией экрана и голосом. Что нужно сказать в видео:

1.  Покажи код транзакции и объясни, как именно ты защитился от **Race Condition** (почему использовал `transaction.atomic`, `select_for_update` или оптимистичную блокировку).
2.  Запусти скрипт (или `curl`), который делает 10 одновременных запросов на списание, и покажи, что баланс не ушел в минус.

***

Могу ли я помочь вам с настройкой вашего проекта на Django/Vue.js или с реализацией одного из этих пунктов?
